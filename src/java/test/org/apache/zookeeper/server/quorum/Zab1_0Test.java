...
public class Zab1_0Test extends ZKTestCase {
    ...
    static void readPacketSkippingPing(InputArchive ia, QuorumPacket qp) throws IOException {
        while (true) {
            ...
            if (qp.getType() != getType(OpOfLeader.PING)) {
                return;
            }
        }
    }

    private static int getType(OpOfLeader op) {
        return op.intType();
    }
    ...
    public void testLeaderInConnectingFollowers() throws Exception {
        ...
        try {
            ...
            // accounted for
            f1.join(leader.getQuorumPeer().getInitLimit() * leader.getQuorumPeer().getTickTime() + 5000);
            f2.join(leader.getQuorumPeer().getInitLimit() * leader.getQuorumPeer().getTickTime() + 5000);
            ...
            try {
                long epoch = leader.getEpochToPropose(leader.getQuorumPeer().getId(), leader.getQuorumPeer().getAcceptedEpoch());
    ...
    public void testPopulatedLeaderConversation(PopulatedLeaderConversation conversation, int ops) throws Exception {
        ...
        try {
            ...
            for (int i = 1; i <= ops; i++) {
                ...
                String path = "/foo-" + i;
                zkDb.processTxn(new TxnHeader(13, 1000 + i, zxid, 30 + i, OpType.create.getValue()),
    ...
    public void testUnnecessarySnap() throws Exception {
        testPopulatedLeaderConversation(new PopulatedLeaderConversation() {
            ...
            public void converseWithLeader(InputArchive ia, OutputArchive oa,
                ...
                Assert.assertEquals(1, l.getQuorumPeer().getAcceptedEpoch());
                Assert.assertEquals(1, l.getQuorumPeer().getCurrentEpoch());
                ...
                QuorumPacket qp = new QuorumPacket(getType(OpOfLeader.FOLLOWERINFO), 1,
                        liBytes, null);
                oa.writeRecord(qp, null);

                readPacketSkippingPing(ia, qp);
                Assert.assertEquals(getType(OpOfLeader.LEADERINFO), qp.getType());
                Assert.assertEquals(ZxidUtils.makeZxid(2, 0), qp.getZxid());
                Assert.assertEquals(ByteBuffer.wrap(qp.getData()).getInt(),
                        0x10000);
                Assert.assertEquals(2, l.getQuorumPeer().getAcceptedEpoch());
                Assert.assertEquals(1, l.getQuorumPeer().getCurrentEpoch());

                byte epochBytes[] = new byte[4];
                final ByteBuffer wrappedEpochBytes = ByteBuffer.wrap(epochBytes);
                wrappedEpochBytes.putInt(1);
                qp = new QuorumPacket(getType(OpOfLeader.ACKEPOCH), zxid, epochBytes, null);
                oa.writeRecord(qp, null);

                readPacketSkippingPing(ia, qp);
                Assert.assertEquals(getType(OpOfLeader.DIFF), qp.getType());
    ...
    public void testNormalFollowerRun() throws Exception {
        testFollowerConversation(new FollowerConversation() {
            ...
            public void converseWithFollower(InputArchive ia, OutputArchive oa,
                ...
                tmpDir.delete();
                tmpDir.mkdir();
                File logDir = f.getZk().getTxnLogFactory().getDataDir().getParentFile();
                File snapDir = f.getZk().getTxnLogFactory().getSnapDir().getParentFile();
                //Spy on ZK so we can check if a snapshot happened or not.
                //f.zk = spy(f.zk);
                try {
                    Assert.assertEquals(0, f.getQuorumPeer().getAcceptedEpoch());
                    Assert.assertEquals(0, f.getQuorumPeer().getCurrentEpoch());
                    ...
                    zkDb.processTxn(new TxnHeader(13, 1313, firstZxid, 33, OpType.create.getValue()), new CreateTxn("/foo", "data1".getBytes(), ZooDefs.Ids.OPEN_ACL_UNSAFE, false, 1));
                    Stat stat = new Stat();
                    Assert.assertEquals("data1", new String(zkDb.getData("/foo", stat, null)));

                    QuorumPacket qp = new QuorumPacket();
                    readPacketSkippingPing(ia, qp);
                    Assert.assertEquals(getType(OpOfLeader.FOLLOWERINFO), qp.getType());
                    ...
                    qp.setType(getType(OpOfLeader.LEADERINFO));
                    ...
                    readPacketSkippingPing(ia, qp);
                    Assert.assertEquals(getType(OpOfLeader.ACKEPOCH), qp.getType());
                    Assert.assertEquals(0, qp.getZxid());
                    Assert.assertEquals(ZxidUtils.makeZxid(0, 0), ByteBuffer.wrap(qp.getData()).getInt());
                    Assert.assertEquals(1, f.getQuorumPeer().getAcceptedEpoch());
                    Assert.assertEquals(0, f.getQuorumPeer().getCurrentEpoch());

                    // Send the snapshot we created earlier
                    qp.setType(getType(OpOfLeader.SNAP));
                    ...
                    verify(f.getZk(), never()).takeSnapshot();

                    qp.setType(getType(OpOfLeader.NEWLEADER));
                    qp.setZxid(ZxidUtils.makeZxid(1, 0));
                    oa.writeRecord(qp, null);

                    // Get the ack of the new leader
                    readPacketSkippingPing(ia, qp);
                    Assert.assertEquals(getType(OpOfLeader.ACK), qp.getType());
                    Assert.assertEquals(ZxidUtils.makeZxid(1, 0), qp.getZxid());
                    Assert.assertEquals(1, f.getQuorumPeer().getAcceptedEpoch());
                    Assert.assertEquals(1, f.getQuorumPeer().getCurrentEpoch());
                    //Make sure that we did take the snapshot now
                    verify(f.getZk()).takeSnapshot(true);
                    Assert.assertEquals(firstZxid, f.getZk().getLastProcessedZxid());
                    ...
                    Assert.assertEquals("data1", new String(f.getZk().getZKDatabase().getData("/foo", stat, watcher)));

                    // The change should happen now
                    qp.setType(getType(OpOfLeader.COMMIT));
                    qp.setZxid(proposalZxid);
                    oa.writeRecord(qp, null);

                    qp.setType(getType(OpOfLeader.UPTODATE));
                    qp.setZxid(0);
                    oa.writeRecord(qp, null);
                    ...
                    Assert.assertEquals("data2", new String(f.getZk().getZKDatabase().getData("/foo", stat, null)));
            ...
            private void proposeSetData(QuorumPacket qp, long zxid, String data, int version) throws IOException {
                qp.setType(getType(OpOfLeader.PROPOSAL));
                qp.setZxid(zxid);
                TxnHeader hdr = new TxnHeader(4, 1414, qp.getZxid(), 55, OpType.setData.getValue());
        ...
        testFollowerConversation(new FollowerConversation() {
            ...
            public void converseWithFollower(InputArchive ia, OutputArchive oa,
                ...
                try {
                    ...
                    // Send a diff
                    qp.setType(getType(OpOfLeader.DIFF));
                    qp.setData(new byte[0]);
                    qp.setZxid(zkDb.getDataTreeLastProcessedZxid());
                    oa.writeRecord(qp, null);
                    ...
                    while (createSessionZxid != f.getZk().getLastProcessedZxid() && (System.currentTimeMillis() - start) < 50) {
                        Thread.sleep(1);
                    }

                    Assert.assertEquals(createSessionZxid, f.getZk().getLastProcessedZxid());
            ...
            private void proposeNewSession(QuorumPacket qp, long zxid, long sessionId) throws IOException {
                qp.setType(getType(OpOfLeader.PROPOSAL));
                qp.setZxid(zxid);
                TxnHeader hdr = new TxnHeader(4, 1414, qp.getZxid(), 55, OpType.createSession.getValue());
    ...
    public void testNormalRun() throws Exception {
        testLeaderConversation(new LeaderConversation() {
            public void converseWithLeader(InputArchive ia, OutputArchive oa, Leader l)
                    throws IOException {
                Assert.assertEquals(0, l.getQuorumPeer().getAcceptedEpoch());
                Assert.assertEquals(0, l.getQuorumPeer().getCurrentEpoch());
                ...
                ByteBufferOutputStream.record2ByteBuffer(li,
                        ByteBuffer.wrap(liBytes));
                QuorumPacket qp = new QuorumPacket(getType(OpOfLeader.FOLLOWERINFO), 0,
                        liBytes, null);
                oa.writeRecord(qp, null);
                ...
                qp = new QuorumPacket(getType(OpOfLeader.ACKEPOCH), 0, new byte[4], null);
                oa.writeRecord(qp, null);
                ...
                readPacketSkippingPing(ia, qp);
                Assert.assertEquals(getType(OpOfLeader.NEWLEADER), qp.getType());
                Assert.assertEquals(ZxidUtils.makeZxid(1, 0), qp.getZxid());
                Assert.assertEquals(1, l.getQuorumPeer().getAcceptedEpoch());
                Assert.assertEquals(1, l.getQuorumPeer().getCurrentEpoch());

                qp = new QuorumPacket(getType(OpOfLeader.ACK), qp.getZxid(), null, null);
                oa.writeRecord(qp, null);

                readPacketSkippingPing(ia, qp);
                Assert.assertEquals(getType(OpOfLeader.UPTODATE), qp.getType());
            }
        });
    }
    ...
    public void testTxnTimeout() throws Exception {
        testLeaderConversation(new LeaderConversation() {
            public void converseWithLeader(InputArchive ia, OutputArchive oa, Leader l)
                ...
                long zxid = l.getZk().getZxid();
                l.propose(new Request(1, 1, OpType.create,
                        new TxnHeader(1, 1, zxid, 1, OpType.create.getValue()),
                        new CreateTxn("/test", "hola".getBytes(), null, true, 0), zxid));

                readPacketSkippingPing(ia, qp);
                Assert.assertEquals(getType(OpOfLeader.PROPOSAL), qp.getType());
                ...
                for (int i = 0; i < (2 * SYNC_LIMIT) + 2; i++) {
                    try {
                        ...
                        qp = new QuorumPacket(getType(OpOfLeader.PING), qp.getZxid(), "".getBytes(), null);
                        oa.writeRecord(qp, null);
    ...
    public void testNormalObserverRun() throws Exception {
        testObserverConversation(new ObserverConversation() {
            ...
            public void converseWithObserver(InputArchive ia, OutputArchive oa,
                ...
                tmpDir.delete();
                tmpDir.mkdir();
                File logDir = o.getZk().getTxnLogFactory().getDataDir().getParentFile();
                File snapDir = o.getZk().getTxnLogFactory().getSnapDir().getParentFile();
                try {
                    Assert.assertEquals(0, o.getQuorumPeer().getAcceptedEpoch());
                    Assert.assertEquals(0, o.getQuorumPeer().getCurrentEpoch());
                    ...
                    QuorumPacket qp = new QuorumPacket();
                    readPacketSkippingPing(ia, qp);
                    Assert.assertEquals(getType(OpOfLeader.OBSERVERINFO), qp.getType());
                    Assert.assertEquals(qp.getZxid(), 0);
                    LearnerInfo learnInfo = new LearnerInfo();
                    ...
                    Assert.assertEquals(1, o.getQuorumPeer().getAcceptedEpoch());
                    Assert.assertEquals(0, o.getQuorumPeer().getCurrentEpoch());
                    ...
                    Assert.assertEquals(ZxidUtils.makeZxid(1, 0), qp.getZxid());
                    Assert.assertEquals(1, o.getQuorumPeer().getAcceptedEpoch());
                    Assert.assertEquals(1, o.getQuorumPeer().getCurrentEpoch());

                    Assert.assertEquals(foo2Zxid, o.getZk().getLastProcessedZxid());
                    ...
                    // Register watcher
                    TrackerWatcher watcher = new TrackerWatcher();
                    Assert.assertEquals("data1", new String(o.getZk()
                    ...
                    qp.setType(getType(OpOfLeader.INFORM));
                    oa.writeRecord(qp, null);
                    ...
                    watcher.waitForChange();
                    Assert.assertEquals("data2", new String(o.getZk()
    ...
    public void testLeaderBehind() throws Exception {
        testLeaderConversation(new LeaderConversation() {
            public void converseWithLeader(InputArchive ia, OutputArchive oa, Leader l)
                ...
                QuorumPacket qp = new QuorumPacket(getType(OpOfLeader.FOLLOWERINFO), ZxidUtils.makeZxid(20, 0),
                        liBytes, null);
                oa.writeRecord(qp, null);
                readPacketSkippingPing(ia, qp);
                Assert.assertEquals(getType(OpOfLeader.LEADERINFO), qp.getType());
    ...
    public void testAbandonBeforeACKEpoch() throws Exception {
        testLeaderConversation(new LeaderConversation() {
            public void converseWithLeader(InputArchive ia, OutputArchive oa, Leader l)
                ...
                Assert.assertEquals(ByteBuffer.wrap(qp.getData()).getInt(),
                        0x10000);
                Thread.sleep(l.getQuorumPeer().getInitLimit() * l.getQuorumPeer().getTickTime() + 5000);
    ...
    private MockLeader createLeader(File tmpDir, QuorumPeer peer)
    ...
    public void testInitialAcceptedCurrent() throws Exception {
        ...
        try {
            ...
            logFactory.append(new Request(1, 1, OpType.error,
                    new TxnHeader(1, 1, zxid, 1, OpType.error.getValue()),
                    new ErrorTxn(1), zxid));
            logFactory.commit();
            ZKDatabase zkDb = new ZKDatabase(logFactory);
            QuorumPeer peer = new QuorumPeer();
    ...
    private static final class MockLeader extends Leader {
        ...
        public boolean isAlive() {
            return super.cnxAcceptor != null && super.cnxAcceptor.isAlive();
        }
    }
