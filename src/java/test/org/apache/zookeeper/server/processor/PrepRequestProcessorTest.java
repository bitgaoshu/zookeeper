...
public class PrepRequestProcessorTest extends ClientBase {
    ...
    public void testMultiOutstandingChange() throws Exception {
        zks.getZKDatabase().getDataTree().createNode("/foo", new byte[0], Ids.OPEN_ACL_UNSAFE, 0, 0, 0, 0);

        Assert.assertNull(zks.getOutstandingChangesForPath().get("/foo"));

        process(Arrays.asList(
                Op.setData("/foo", new byte[0], -1)));

        ChangeRecord cr = zks.getOutstandingChangesForPath().get("/foo");
        ...
        process(Arrays.asList(
                Op.delete("/foo", -1)));
        cr = zks.getOutstandingChangesForPath().get("/foo");
        Assert.assertEquals("Record zxid wasn't set correctly",
                2, cr.zxid);
    ...
    public void testMultiRollbackNoLastChange() throws Exception {
        zks.getZKDatabase().getDataTree().createNode("/foo", new byte[0], Ids.OPEN_ACL_UNSAFE, 0, 0, 0, 0);
        zks.getZKDatabase().getDataTree().createNode("/foo/bar", new byte[0], Ids.OPEN_ACL_UNSAFE, 0, 0, 0, 0);
