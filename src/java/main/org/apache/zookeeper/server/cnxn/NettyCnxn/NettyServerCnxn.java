...
package org.apache.zookeeper.server.cnxn.NettyCnxn;
...
public class NettyServerCnxn extends ServerCnxn {
    ...
    public void close() {
        ...
        final Set<ServerCnxn> cnxns = factory.getCnxns();
        synchronized(cnxns){
            // if this is not in cnxns then it's already closed
            if (!cnxns.remove(this)) {
                if (LOG.isDebugEnabled()) {
                    LOG.debug("cnxns size:" + factory.getNumAliveConnections());
                }
                return;
            }
            if (LOG.isDebugEnabled()) {
